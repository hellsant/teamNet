var app = angular.module('TeamNet', ['ngTable', 'ui.bootstrap', 'app.config'])
var custom = false;
...
app.controller('TableController', function ($scope, $http, $filter, $modal, ngTableParams, RESULTS_CONFIG, URLs) {
    ...
    $http.get(URLs.SERVICE + URLs.RESULT + '/' + email)
    ...
    function addEvaluator(competency, evaluator) {
        if (!(competency.results[0].evaluator === evaluator)) {
            var newResult = [{ evaluator: evaluator, value: competency.resultAvg }];
    ...
    $scope.initAverages = function (competency) {
        var sum = 0;
        for (var result in competency.results) {
            var val = competency.results[result].value * 1;
            sum += val;
    }
        competency.resultAvg = (sum / competency.results.length).toFixed(config.DECIMAL);
    ...
    $scope.progressAvg = function (competency, index) {
        ...
        var first = { value: competency.resultAvg, 
        ...
        for (var i = 0; i < types.length; i++) {
            ...
            if (competency.resultAvg <= expectedPercentage) {
                competency.competencyStacked = [];
                typeBar = types[i];
                first = { value: competency.resultAvg, 
                          percentage: competency.percentage,
                          type: typeBar.name, 
                          show: true 
                         };
                ...
                if (competency.needAvg != 0) {
                    second = { value: competency.needAvg,
    ...
    $scope.isDiffSuggested = function (competency) {
        return competency.expectedAvg - competency.resultAvg >= max_difference;
    }
    ...
    $scope.openModal = function (competency) {
        ...
        $scope.changeSelection(competency);
    };
});
...
function updateTask(oldTask, newTask, filter) {
    ...
    oldTask.idUser = newTask.idUser;
    oldTask.idPoint = newTask.idPoint;
    oldTask.initialDate = newTask.initialDate;
    oldTask.finalDate = newTask.finalDate;
    ...
    var date = new Date(newTask.finalDate);
    oldTask.finalDate = filter('date')(date, 'yyyy-MM-dd');
}
